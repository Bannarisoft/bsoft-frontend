...
import { CreateUomProps } from "../../../../types/inventoryTypes";
import CreateInveUom from "../../../molecules/Inventory/Master/CreateUom";
const UomInventoryPage = () => {
  ...
  const [uomInput, setUomInput] = useState<CreateUomProps>({
    code: "",
    uomName: "",
    uomTypeId: 0,
    uomType: "",
  ...
  const GetUomList = async () => {
    try {
      const { endpoint, method } = InventoryInventoryConfig.Uom;
  ...
  const AddUom = async () => {
    ...
    try {
      const { endpoint, method } = InventoryInventoryConfig.Uom.AddUom;
      const response = await Apirequest(
        endpoint,
        method,
        body,
  ...
  const UpdateUom = async () => {
    ...
    try {
      const { endpoint, method } = InventoryInventoryConfig.Uom.UpdateUom;
      const response = await Apirequest(
        endpoint,
        method,
        body,
  ...
  const { data: uomType } = useDataFetchHook(
    InventoryInventoryConfig.Uom.Misc.endpoint.replace("{type}", "uomtype"),
    InventoryInventoryConfig.Uom.Misc.method,
    "inventory"
  );
  const handleAutocompleteChange = (
    ...
    if (field === "uomType") {
      if (!value || Array.isArray(value)) {
        ...
        setUomInput((prev) => ({ ...prev, uomTypeId: value.id }));
        setError([]);
      }
    }
  };
  ...
  const DeleteUom = async () => {
    try {
      ...
      const { endpoint, method } = InventoryInventoryConfig.Uom.DeleteUom;
      const response = await Apirequest(
  ...
  return (
    <>
      ...
      >
        <Box>
          <IconBreadcrumbs parent={"Inventory"} child={"UOM"} path="" />
        </Box>
        <Box
          display={"flex"}
          justifyContent={"space-between"}
...
export default UomInventoryPage;
